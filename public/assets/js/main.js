...
function finalizeGenerateWave(files,file, i,nextWave,thisWave) {
...
$(function() {
    ...
    setInterval(function() {
        ...
        if (isLoggedIn && !window.notificationIsChecking) {
            ...
            $.ajax({
                url : buildLink('check/notification', [{key : 'time', value: lastTime},{key:'push',value:push}]),
                success : function(r) {
                    if(r === 'login') {
                        isLoggedIn = false;
                        return;
...
function setupBrowserNotification() {
    ...
    let promiseChain = new Promise((resolve, reject) => {
        ...
        .then((result) => {
            if (result === 'granted') {
                return navigator.serviceWorker.register(baseUrl+'/service-worker.js')
                    .then(function(registration) {
                        window.browserNotification = registration;
