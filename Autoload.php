...
class Autoload
{
	...
	/**
	 * Recursively check the override directory for an override
	 *
	 * @param string $classname
	 */
	private function overrideFileExists($classname)
	{
		$all_overrides  = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this->root_dir.'override'));
		$override = new RegexIterator($all_overrides, '/'.$classname.'\.php/');
		// Because the above will match FrontController.php to Controller.php and files are in subdirectories
		// We need to now explicitly match the file basename.
		if (is_object($override))
			foreach ($override as $entry)
				if ($entry->getBasename() == $classname.'.php')
					return true;
		return false;
	}
	...
	public function load($classname)
	{
		...
		if (!isset($this->index[$classname])
			|| ($this->index[$classname] == '' && $this->overrideFileExists($classname))
